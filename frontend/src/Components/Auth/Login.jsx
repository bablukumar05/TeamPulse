...
const Login = ({ onClose }) => {
  ...
  const cardContent = (
    <div className={`relative z-10 w-full ${isRegistering ? 'max-w-md' : 'max-w-sm'} rounded-xl border border-slate-800/90 bg-[#111827] p-5 sm:p-6 shadow-2xl transition-all duration-200`}>
      {/* Top right working close button */}
      <button
        type="button"
        onClick={handleClose}
        className="absolute top-3.5 right-3.5 flex h-7 w-7 items-center justify-center rounded-lg text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800/80 border border-transparent hover:border-zinc-700/60 transition-all cursor-pointer z-20 group"
        title="Close"
        aria-label="Close"
      >
      ...
      <div className="flex flex-col items-center">
        ...
        <form onSubmit={submitHandler} className="w-full space-y-3">
          ...
          <button
            ...
            className="mt-3.5 w-full rounded-lg bg-zinc-100 hover:bg-white text-zinc-950 py-2 text-xs sm:text-sm font-medium transition-colors shadow-sm disabled:opacity-50 flex items-center justify-center cursor-pointer"
            type="submit"
          >
            {isForgotPassword
  ...
  return (
    <div className="relative flex min-h-screen w-full items-center justify-center bg-[#0B0F19] p-3 sm:p-4">
      {cardContent}
    </div>
  );
};
